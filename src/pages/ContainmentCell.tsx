...
const BEAST = {
  ...
  lineage: {
    ...
    mutations: [
      ...
      "GEN 2: ACQUIRED TOXIC RESISTANCE",
      "GEN 3: NEURAL AGGRESSION AMPLIFIED",
    ],
  },
  memories: [
...
const StatBar = ({ label, value, color }: { label: string; value: number; color: string }) => (
  <div className="flex flex-col items-center gap-1">
    ...
    <span className="font-data text-[8px] text-muted-foreground tracking-wider text-center w-12">
      {label}
    </span>
...
const DnaHelix = () => {
  ...
  return (
    <svg viewBox="0 0 120 200" className="w-24 h-40 mx-auto dna-rotate">
      {Array.from({ length: rungs }).map((_, i) => {
        ...
        return (
          <g key={i} onClick={() => setActiveSegment(i === activeSegment ? null : i)} className="cursor-pointer">
            ...
            <line
              x1={40 + offset} y1={y} x2={80 - offset} y2={y}
              stroke={isActive ? "hsl(var(--glitch-cyan))" : "hsl(var(--static-gray) / 0.5)"}
      ...
      {/* Backbone strands */}
      <path
        d={Array.from({ length: rungs }).map((_, i) => {
          const y = 15 + i * 22;
          const offset = Math.sin((i / rungs) * Math.PI * 2) * 20;
          return `${i === 0 ? "M" : "L"}${40 + offset},${y}`;
        }).join(" ")}
        stroke="hsl(var(--neon-claw) / 0.4)" strokeWidth="1.5" fill="none"
      />
      <path
        d={Array.from({ length: rungs }).map((_, i) => {
          const y = 15 + i * 22;
        ...
        stroke="hsl(var(--glitch-cyan) / 0.4)" strokeWidth="1.5" fill="none"
      />
    </svg>
  );
};
  ...
  return (
    <svg viewBox="0 0 160 160" className="w-full max-w-[200px] mx-auto">
      ...
      {keys.map((k, i) => {
        ...
        return (
          <circle key={k}
            cx={cx + Math.cos(angles[i]) * r * val}
            cy={cy + Math.sin(angles[i]) * r * val}
            r="3" fill="hsl(var(--neon-claw))"
          />
        );
      })}
    </svg>
    ...
    <div className="min-h-screen bg-background containment-grid">
      ...
      <div className="border-b border-border bg-card/80 px-4 py-3 flex items-center justify-between">
        ...
        <div className="font-data text-[10px] text-muted-foreground tracking-wider">
          GEN: {BEAST.generation} | XP: {BEAST.xp}/{BEAST.xpNext}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-49px)]">
        ...
        <div className="lg:w-[60%] relative flex items-center justify-center p-6 beast-viewer">
          <div className="observation-frame relative w-full max-w-xl aspect-square bg-card/30 overflow-hidden stasis-scan">
            ...
            {/* Rarity badge */}
            <div
              className="absolute top-4 right-4 z-20 font-marker text-lg text-neon-claw tracking-wider px-3 py-1 border-2 border-neon-claw/60 bg-neon-claw/10"
            ...
            <div className="absolute bottom-4 left-4 z-20 flex items-center gap-2 orb-pulse">
              <div
                ...
                style={{ background: moodColors[BEAST.mood], boxShadow: `0 0 12px ${moodColors[BEAST.mood]}` }}
              />
              <span className="font-data text-[9px] tracking-[0.2em]" style={{ color: moodColors[BEAST.mood] }}>
                {BEAST.mood}
              </span>
            </div>
            ...
            {/* CRYOGENIC STASIS label */}
            <div className="absolute top-4 left-4 z-20">
              <span className="font-data text-[9px] text-glitch-cyan/60 tracking-[0.3em]">
                CRYOGENIC STASIS
              </span>
            </div>
          </div>
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={`flex-1 px-2 py-3 font-display text-[10px] tracking-[0.15em] transition-colors border-b-2 ${
                  activeTab === tab
                    ? "text-neon-claw border-neon-claw bg-neon-claw/5"
          ...
          <div className={`flex-1 overflow-y-auto p-5 ${tabGlitch ? "tab-glitch" : ""}`}>
            {activeTab === "BIOLOGY" && (
              <div className="space-y-6">
                {/* DNA Helix */}
                <div>
                  <h3 className="font-display text-[10px] text-muted-foreground tracking-[0.2em] mb-3">
                    DNA STRUCTURE
                  </h3>
                  <DnaHelix />
                  <p className="font-data text-[8px] text-muted-foreground/60 text-center mt-1 tracking-wider">
                ...
                {/* Hydraulic readout bars */}
                <div>
                  <h3 className="font-display text-[10px] text-muted-foreground tracking-[0.2em] mb-3">
                    HYDRAULIC READOUTS
                  </h3>
                  <div className="flex justify-between px-2">
                    <StatBar label="AGGR" value={BEAST.stats.aggression} color="hsl(var(--neon-claw))" />
                    <StatBar label="CALC" value={BEAST.stats.calculated} color="hsl(var(--glitch-cyan))" />
                    <StatBar label="UNST" value={BEAST.stats.unstable} color="hsl(var(--rust-gold))" />
                    <StatBar label="LOYL" value={BEAST.stats.loyal} color="hsl(var(--toxic-shard))" />
                    <StatBar label="VAIN" value={BEAST.stats.vain} color="hsl(var(--neon-claw))" />
                  </div>
                </div>

                {/* Rage quit probability */}
                <div className="border border-rust-gold/30 bg-rust-gold/5 p-3">
                  <span className="font-data text-[10px] text-rust-gold tracking-[0.2em]">
                    RAGE QUIT PROBABILITY: {BEAST.rageQuitProb}%
                  </span>
                  <div className="mt-2 h-1.5 bg-background overflow-hidden">
                    <div
                      className="h-full bg-rust-gold stat-bar-fill"
              ...
              <div className="space-y-6">
                    ...
                    {BEAST.battles.map((b) => (
                      <div key={b.id} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={b.outcome === "VICTORY" ? "text-toxic-shard" : "text-neon-claw"}>
                            {b.outcome}
                          </span>
                ...
                <div>
                  <h3 className="font-display text-[10px] text-muted-foreground tracking-[0.2em] mb-3 flex items-center gap-2">
                    <BrokenChainIcon size={14} className="text-neon-claw" />
                    GRUDGES DETECTED
                  </h3>
            ...
            {activeTab === "LINEAGE" && (
              <div className="space-y-6">
                ...
                <div className="text-center border border-border p-4 bg-secondary/10">
                  <div className="font-display text-4xl text-foreground tracking-wider">GEN: {BEAST.generation}</div>
                  <div className="font-data text-[9px] text-muted-foreground tracking-[0.2em] mt-1">ITERATION DEPTH</div>
                </div>

                {/* Family tree as circuit traces */}
                <div>
                  <h3 className="font-display text-[10px] text-muted-foreground tracking-[0.2em] mb-3">
                    GENETIC ORIGIN
                  </h3>
                  <svg viewBox="0 0 300 120" className="w-full">
                    ...
                    <text x="70" y="29" fill="hsl(var(--glitch-cyan))" fontSize="8" textAnchor="middle" fontFamily="'Share Tech Mono', monospace">
                      {BEAST.lineage.parent1}
                    </text>
                    {/* Parent 2 */}
                    <rect x="170" y="10" width="120" height="30" stroke="hsl(var(--neon-claw))" strokeWidth="1" fill="hsl(var(--neon-claw) / 0.05)" />
                    <text x="230" y="29" fill="hsl(var(--neon-claw))" fontSize="8" textAnchor="middle" fontFamily="'Share Tech Mono', monospace">
                      {BEAST.lineage.parent2}
                    </text>
                    {/* Connection traces */}
                    ...
                    <text x="150" y="99" fill="hsl(var(--rust-gold))" fontSize="9" textAnchor="middle" fontFamily="'Chakra Petch', sans-serif" fontWeight="bold">
                      {BEAST.name}
                    </text>
              ...
              <div className="space-y-6">
                ...
                <div className="border border-glitch-cyan/30 bg-glitch-cyan/5 p-4">
                  ...
                  <div className="h-2 bg-background overflow-hidden">
                    <div
                      className="h-full bg-glitch-cyan stat-bar-fill"
                    ...
                    {BEAST.memories.map((mem, i) => (
                      <div key={i} className="bg-background border border-border p-3">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 font-data text-[8px] tracking-wider">
                          ...
                          <span className="text-muted-foreground">OUTCOME:{" "}
                            <span className={mem.outcome === "VICTORY" ? "text-toxic-shard" : "text-neon-claw"}>
                              {mem.outcome}
                            </span>
                          </span>
                          <span className="text-muted-foreground">EMOTION:{" "}
                            <span className="text-rust-gold">{mem.emotion}</span>
                          </span>
                        </div>
                        <div className="mt-2 h-1 bg-secondary overflow-hidden">
                          <div
                            className="h-full bg-toxic-shard/60"
                            style={{ width: `${mem.learning}%` }}
                          />
                        </div>
                      </div>
                    ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 border-t border-border bg-card/95 backdrop-blur-sm px-4 py-3 z-30">
        <div className="flex gap-3 max-w-5xl mx-auto">
          ...
          >
            <CrossedClawsIcon size={18} className="text-current" />
            INITIATE COMBAT
          </button>
          ...
          <button className="hydraulic-press flex-1 border-2 border-toxic-shard bg-toxic-shard/10 py-3 font-display text-[10px] font-bold uppercase tracking-[0.15em] text-toxic-shard transition-all hover:bg-toxic-shard hover:text-accent-foreground flex items-center justify-center gap-2">
            <DnaGearIcon size={16} className="text-current" />
            BREEDING PROTOCOL
          </button>
          ...
          <button className="hydraulic-press flex-1 border-2 border-rust-gold/40 bg-rust-gold/5 py-3 font-display text-[10px] font-bold uppercase tracking-[0.15em] text-rust-gold/50 cursor-not-allowed flex items-center justify-center gap-2">
            <CircuitLockIcon size={16} className="text-current" />
            FORCE EVOLUTION
          </button>
          ...
          <button className="hydraulic-press flex-1 border-2 border-glitch-cyan bg-glitch-cyan/10 py-3 font-display text-[10px] font-bold uppercase tracking-[0.15em] text-glitch-cyan transition-all hover:bg-glitch-cyan hover:text-accent-foreground flex items-center justify-center gap-2">
            <BladeTagIcon size={16} className="text-current" />
            LIST SPECIMEN
          </button>
        </div>
      </div>
